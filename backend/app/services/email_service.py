...
Reemplaza Flask-Mail para envío de emails transaccionales
"""
import os
import resend
...
def send_verification_email(email: str, name: str, code: str) -> bool:
    ...
    """
    try:
        # URL del portfolio (usar variable de entorno en producción)
        portfolio_url = os.getenv('PORTFOLIO_URL', 'http://localhost:4200')
        
        params = {
            "from": f"Carlos Sánchez Portfolio <onboarding@resend.dev>",
            "to": email,
...
<body>
    <div class="container">
        ...
        <div class="footer">
            ...
            <p>© {datetime.now().year} Carlos Sánchez - Desarrollador Full Stack</p>
        </div>
    </div>
</body>
</html>
...
{portfolio_url}/contact
        ...
        current_app.logger.info(f"Email sent via Resend to {email}: {email_response}")
        return True
        
    except Exception as e:
        current_app.logger.error(f"Resend email failed for {email}: {str(e)}")
        raise e
